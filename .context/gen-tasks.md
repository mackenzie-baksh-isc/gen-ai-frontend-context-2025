...
To guide an AI assistant in creating a detailed, step-by-step task list in Markdown format based on an existing Product Requirements Document (PRD). The task list should guide a developer through implementation & structure the delivery/phases based on feature dependency for ex. if there is ever an auth layer that needs a login page, login page must be created first before auth layer.
